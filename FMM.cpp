...
FMM::FMM(const ob::SpaceInformationPtr& si) : ob::Planner(si, "FMM") {
...
void FMM::initializeParameters() {
    ...
    reallowbound = bounds.low[0];
    realhighbound = bounds.high[0];
    lowbound = reallowbound;
    highbound = realhighbound;
//    numGrid = 2;
//TODO,change back
    numGrid = 4;
    lastnumGrid = 0;    
...
//        numGrid = 2*lastnumGrid;
    ...
    bool isSolved = computeDistanceMap(ptc);
    if (isSolved) {
        Vector goalV, startV;
...
bool FMM::computeDistanceMap(const ob::PlannerTerminationCondition &ptc){
    Timer timer;
    ...
    if(status.numValues() > FMM_NUM_LIMIT){
    	return false;
    }
	//build queue
...
void FMM::setLowerBoundOffset(double offset) {
	this->lowbound = reallowbound + offset;
	this->highbound = realhighbound;
    grid_resolution = (highbound - lowbound) / (double)numGrid;
    gridoffset = offset;
}
...
bool FMM::isStateValid(const Vector& state) {
    ...
    for(int i = 0; i < nD; i++)
    {
    	if(state[i] < this->reallowbound || state[i] > this->realhighbound){
    		return false;
    	}
    ...
    if(stateChecker != NULL){
    	result = stateChecker->isValid(omplstate);
    }else{
    	result = si_->getStateValidityChecker()->isValid(omplstate);
    }
